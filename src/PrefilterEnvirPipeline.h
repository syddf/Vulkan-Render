#ifndef _PREFILTER_ENVIRONMENT_PIPELINE_H_
#define _PREFILTER_ENVIRONMENT_PIPELINE_H_

#include "VulkanPipeline.h"

class PrefilterEnvironmentPipeline : public IRenderingPipeline
{
public:
	PrefilterEnvironmentPipeline(VulkanDevice* device, VkQueue queue, VulkanMesh * mesh, TextureCube * skyboxTexture)
	{
		device_ = device;
		queue_ = queue;
		mesh_ = mesh;
...
public:
	VkPipeline CreateGraphicsPipeline()
	{
		...
		std::vector<VkPipelineShaderStageCreateInfo> pipelineShaderStageCreateInfo =
		{
			...
			VulkanInitializer::InitShaderStageCreateInfo(VK_SHADER_STAGE_FRAGMENT_BIT , GetAssetPath() + "shaders/prefilterEnvir.spv" , device_)
		};
	...
	void SetupCommandBuffer(VkCommandBuffer & commandBuffer, bool startRenderPass, bool endRenderPass)
	{
		...
		{
			float roughness = (float)mipLevel / float(mip_levels_ - 1);
			...
			for (int arrayLayer = 0; arrayLayer < 6; arrayLayer++)
			{
				...
				SetPushConstantData(glm::perspective((float)(PI / 2.0), 1.0f, 0.1f, 512.0f) * matrices[arrayLayer], roughness , 32);
				...
				prefilter_envir_map_image_->CopyImageToImage(queue_, offscreen_tmp_image_, prefilter_envir_map_image_, viewport.width, viewport.height, 0, 1, 0, 1, arrayLayer, 1, mipLevel, mip_levels_, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, commandBuffer);
			}
			viewport.width = viewport.width * 0.5f;
			viewport.height = viewport.height * 0.5f;
		}

		prefilter_envir_map_image_->TranslateImageLayout(VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, queue_);
	}
	...
	VkRenderPass CreateRenderPass()
	{
		...
		{
			VulkanInitializer::InitAttachmentDescription(VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_STORE),
		};
	...
	void PrepareResources()
	{
		...
		offscreen_tmp_image_ = new VulkanImage(device_, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_UNDEFINED, dim_, dim_, VK_IMAGE_ASPECT_COLOR_BIT, 1, 1, false);
		offscreen_tmp_image_->TranslateImageLayout(VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, queue_);
		prefilter_envir_map_image_ = new VulkanImage(device_, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, VK_IMAGE_LAYOUT_UNDEFINED, dim_, dim_, VK_IMAGE_ASPECT_COLOR_BIT, 6, mip_levels_, true);
		prefilter_envir_map_image_->TranslateImageLayout(VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, queue_, VK_NULL_HANDLE, 6, mip_levels_);
		InitDesc();
		CreateRenderPass();
	...
	void SetPushConstantData(glm::mat4 mvp, float roughness , uint32_t sampleCount )
	{
		PushConstantDataUniform.mvp = mvp;
		PushConstantDataUniform.roughness = roughness ;
		PushConstantDataUniform.numSamples = sampleCount ;
	}
	void UpdateData() {};
	...
	VulkanImage* GetPrefilterEnvirMapImage() const { return prefilter_envir_map_image_; }
...
private:
	...
	VulkanImage * prefilter_envir_map_image_;
