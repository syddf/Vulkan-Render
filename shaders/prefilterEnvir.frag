...
vec3 prefilterEnvirMap(vec3 R , float roughness)
{
	vec3 N = R;
	vec3 V = R;
	vec3 color = vec3(0.0);
	...
	for(uint i = 0u; i < push_constant.numSamples; i++) {
		vec2 Xi = hammersley2d(i, push_constant.numSamples);
		...
		if(dotNL > 0.0) {
			...
			float omegaS = 1.0 / (float(push_constant.numSamples) * pdf);
...
void main()
{
	...
	outColor = vec4( prefilterEnvirMap( N , push_constant.roughness ), 1.0f); 
}